...
namespace InfraSim.Tests
{
    ...
    {
        [Fact]
        public void RouteTraffic_ShouldSendRequestsOnlyToCDNServers()
        {
            // Arrange
            var mockServer1 = new Mock<IServer>();
            mockServer1.Setup(s => s.ServerType).Returns(ServerType.CDN); // Configura o ServerType como CDN
...
|
            ...
            // Act
            cdnTrafficRouting.RouteTraffic(100);

            // Assert
            mockServer1.Verify(s => s.HandleRequests(100), Times.Once); // Verifica se o servidor do tipo CDN recebeu as solicitações
